...
static void hll_aggregation_restriction_hook(PlannerInfo *root,
...
#endif
static Oid FunctionOid(const char *schemaName, const char *functionName,
    ...
    if (stage == UPPERREL_GROUP_AGG || stage == UPPERREL_FINAL) {
      ...
      foreach (pathCell, output_rel->pathlist) {
        ...
        if (path->pathtype == T_Agg &&
            ((AggPath *)path)->aggstrategy == AGG_HASHED) {
...
static void InitializeHllAggregateOids(void) {
  ...
  for (addAggArgumentCounter = 1; addAggArgumentCounter < HLL_AGGREGATE_COUNT;
       addAggArgumentCounter++) {
    aggregateOid =
        FunctionOid(hllSchemaName, aggregateName, addAggArgumentCounter, true);
...
static Oid FunctionOid(const char *schemaName, const char *functionName,
  ...
  List *qualifiedFunctionNameList =
      stringToQualifiedNameList(qualifiedFunctionName
#if PG_VERSION_NUM >= 160000
                                ,
                                NULL
#endif
  ...
  functionList = FuncnameGetCandidates(
      qualifiedFunctionNameList, argumentCount, argumentList, findVariadics,
      findDefaults, includeOutArguments, true, &fgcFlags);
#elif PG_VERSION_NUM >= 140000
  const bool includeOutArguments = false;

  functionList = FuncnameGetCandidates(qualifiedFunctionNameList, argumentCount,
                                       argumentList, findVariadics,
                                       findDefaults, includeOutArguments, true);
#else
  functionList =
      FuncnameGetCandidates(qualifiedFunctionNameList, argumentCount,
                            argumentList, findVariadics, findDefaults, true);
#endif
...
static void MaximizeCostOfHashAggregate(Path *path) {
  List *varList =
      pull_var_clause((Node *)path->pathtarget->exprs, PVC_INCLUDE_AGGREGATES);
...
static void RegisterConfigVariables(void) {
  DefineCustomBoolVariable(
      ...
      NULL, &ForceGroupAgg, false, PGC_USERSET, 0, NULL, NULL, NULL);
}

/* _PG_fini uninstalls extension hooks */
void _PG_fini(void) { create_upper_paths_hook = previous_upper_path_hook; }
#endif

static int32 typmod_log2m(int32 typmod) {
  return (typmod >> (TYPMOD_BITS - LOG2M_BITS)) & MAX_BITVAL(LOG2M_BITS);
}
...
static int32 typmod_expthresh(int32 typmod) {
  return (typmod >>
          (TYPMOD_BITS - LOG2M_BITS - REGWIDTH_BITS - EXPTHRESH_BITS)) &
         MAX_BITVAL(EXPTHRESH_BITS);
}

static int32 typmod_sparseon(int32 typmod) {
  return (typmod >> (TYPMOD_BITS - LOG2M_BITS - REGWIDTH_BITS - EXPTHRESH_BITS -
                     SPARSEON_BITS)) &
...
static void check_modifiers(int32 log2m, int32 regwidth, int64 expthresh,
                            int32 sparseon);
static void check_unpacked_modifiers(int32 log2m, int32 regwidth,
                                     int64 expthresh, int32 sparseon);
// If expthresh == -1 (auto select expthresh) determine
// the expthresh to use from nbits and nregs.
//
static size_t expthresh_value(int64 expthresh, size_t nbits, size_t nregs) {
  if (expthresh != -1) {
...
/*
 * The result of mse_nelem_max()/msc_regs_idx_limit() is a const in a specific
 * platform. But we couldn't hard code it explicitly because we do not know
 * current alignment schema when compiling it.
 */
static size_t mse_nelem_max(void) {
...
static size_t msc_regs_idx_limit(void) {
  multiset_t *o_msp = NULL;
  uint8_t *const ms_data_limit =
      ((uint8_t *)&o_msp->ms_data) + sizeof(o_msp->ms_data);
  return (ms_data_limit - (uint8_t *)&o_msp->ms_data.as_comp.msc_regs[0]) /
         sizeof(compreg_t);
}

static uint32_t bitstream_unpack(bitstream_read_cursor_t *brcp) {
  uint32_t retval;
...
static void compressed_unpack(compreg_t *i_regp, size_t i_width, size_t i_nregs,
                              uint8_t const *i_bitp, size_t i_size,
                              uint8_t i_vers) {
  size_t bitsz;
  size_t padsz;
...
static void sparse_unpack(compreg_t *i_regp, size_t i_width, size_t i_log2nregs,
                          size_t i_nfilled, uint8_t const *i_bitp,
                          size_t i_size) {
  size_t bitsz;
  size_t padsz;
  size_t chunksz;
...
static void bitstream_pack(bitstream_write_cursor_t *bwcp, uint32_t val) {
  size_t bits_left_in_first_byte = 8 - bwcp->bwc_used;
  size_t bits_to_write_after_first_byte =
  ...
  if (bwcp->bwc_nbits < bits_left_in_first_byte) {
    *bwcp->bwc_curp =
        (*bwcp->bwc_curp) |
        ((uint8_t)(val << (bits_left_in_first_byte - bwcp->bwc_nbits)));
  ...
  *bwcp->bwc_curp =
      (*bwcp->bwc_curp) |
      ((uint8_t)(val >> (bwcp->bwc_nbits - bits_left_in_first_byte)));
  ...
  if (remainder_bits_after_full_bytes > 0) {
    ...
    // no OR here because byte is guaranteed to be completely unused
    *bwcp->bwc_curp = ((uint8_t)val & mask)
                      << (8 - remainder_bits_after_full_bytes);
...
static void compressed_pack(compreg_t const *i_regp, size_t i_width,
                            size_t i_nregs, uint8_t *o_bitp, size_t i_size,
                            uint8_t i_vers) {
  size_t bitsz;
  size_t padsz;
...
static void sparse_pack(compreg_t const *i_regp, size_t i_width, size_t i_nregs,
                        size_t i_log2nregs, size_t i_nfilled, uint8_t *o_bitp,
                        size_t i_size) {
  size_t bitsz;
  size_t padsz;
...
static void check_metadata(multiset_t const *i_omp, multiset_t const *i_imp) {
...
static void copy_metadata(multiset_t *o_msp, multiset_t const *i_msp) {
...
static void compressed_add(multiset_t *o_msp, uint64_t elem) {
...
static void compressed_explicit_union(multiset_t *o_msp,
                                      multiset_t const *i_msp) {
  ms_explicit_t const *msep = &i_msp->ms_data.as_expl;
  for (size_t ii = 0; ii < msep->mse_nelem; ++ii)
    compressed_add(o_msp, msep->mse_elems[ii]);
}

static void explicit_to_compressed(multiset_t *msp) {
...
static int element_compare(void const *ptr1, void const *ptr2) {
...
static char *multiset_tostring(multiset_t const *i_msp) {
  char expbuf[256];
  char *retstr;
  size_t len;
  size_t used;
  ...
  case MST_COMPRESSED: {
    ...
    used += snprintf(retstr, len,
                     ...
                     "sparseon=%zu:",
                     numfilled(i_msp), nregs, nbits, expbuf, sparseon);

    for (size_t rr = 0; rr < nrows; ++rr) {
      size_t pos = 0;
      pos = snprintf(linebuf, sizeof(linebuf), "\n%4zu: ", ndx);
      for (size_t cc = 0; cc < rowsz; ++cc) {
        pos += snprintf(&linebuf[pos], sizeof(linebuf) - pos, "%2d ",
                        mscp->msc_regs[ndx]);
        ++ndx;
      }
...
static void explicit_validate(multiset_t const *i_msp,
                              ms_explicit_t const *i_msep) {
  ...
  for (size_t ii = 0; ii < i_msep->mse_nelem - 1; ++ii) {
    if (element_compare(&i_msep->mse_elems[ii], &i_msep->mse_elems[ii + 1]) !=
        -1) {
      char *buf = multiset_tostring(i_msp);

      ereport(ERROR,
              (errcode(ERRCODE_DATA_EXCEPTION),
               errmsg("duplicate or descending explicit elements: %s", buf)));
...
static size_t upper_bound(const uint64_t elements[], size_t n, uint64_t value) {
  size_t low = 0;
  size_t high = n;
...
static void multiset_add(multiset_t *o_msp, uint64_t element) {
  // WARNING!  This routine can change the type of the multiset!

  size_t expval =
      expthresh_value(o_msp->ms_expthresh, o_msp->ms_nbits, o_msp->ms_nregs);
  ...
  case MST_EXPLICIT: {
    ...
    if (msep->mse_nelem >= expval || msep->mse_nelem >= mse_nelem_max()) {
      ...
      if (insert_pos < msep->mse_nelem) {
        memmove(&msep->mse_elems[insert_pos + 1], &msep->mse_elems[insert_pos],
...
static void explicit_union(multiset_t *o_msp, ms_explicit_t const *i_msep) {
  ...
  for (size_t ii = 0; ii < i_msep->mse_nelem; ++ii) {
    ...
    switch (o_msp->ms_type) {
    case MST_EXPLICIT:
      if (bsearch(&element, msep->mse_elems, orig_nelem, sizeof(uint64_t),
                  element_compare))
        continue;
  ...
  if (o_msp->ms_type == MST_EXPLICIT) {
    // Resort the elements.
    qsort(msep->mse_elems, msep->mse_nelem, sizeof(uint64_t), element_compare);
  }
}

static void unpack_header(multiset_t *o_msp, uint8_t const *i_bitp,
                          uint8_t vers, uint8_t type) {
...
static uint8_t multiset_unpack(multiset_t *o_msp, uint8_t const *i_bitp,
                               size_t i_size, uint8_t *o_encoded_type) {
  ...
  case MST_EMPTY:
    if (vers == 1) {
      ...
      check_modifiers(o_msp->ms_log2nregs, o_msp->ms_nbits, o_msp->ms_expthresh,
                      o_msp->ms_sparseon);
    } else {
      ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION),
                      errmsg("unsupported empty version %d", (int)vers)));
    }
  ...
  case MST_EXPLICIT:
    if (vers == 1) {
      ...
      ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION),
                      errmsg("unsupported explicit version %d", (int)vers)));
    }
    break;
  ...
  case MST_COMPRESSED:
    if (vers == 1) {
      ...
      // Fill the registers.
      compressed_unpack(o_msp->ms_data.as_comp.msc_regs, nbits, nregs,
                        &i_bitp[hdrsz], i_size - hdrsz, vers);
    } else {
      ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION),
                      errmsg("unsupported compressed version %d", (int)vers)));
    }
    break;
  ...
  case MST_UNDEFINED:
    if (vers == 1) {
      ...
      ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION),
                      errmsg("unsupported undefined version %d", (int)vers)));
    }
  ...
  case MST_SPARSE:
    if (vers == 1) {
      ...
      if (i_size < hdrsz) {
        ...
        // Fill the registers.
        sparse_unpack(mscp->msc_regs, nbits, log2nregs, nfilled, &i_bitp[hdrsz],
                      i_size - hdrsz);
      }
    } else {
      ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION),
                      errmsg("unsupported sparse version %d", (int)vers)));
    }
    break;
...
static size_t pack_header(uint8_t *o_bitp, uint8_t vers, uint8_t type,
                          size_t nbits, size_t log2nregs, int64 expthresh,
                          size_t sparseon) {
  size_t ndx = 0;
...
static void multiset_pack(multiset_t const *i_msp, uint8_t *o_bitp,
                          size_t i_size) {
  ...
  switch (i_msp->ms_type) {
  case MST_EMPTY:
    pack_header(o_bitp, vers, MST_EMPTY, nbits, log2nregs, expthresh, sparseon);
    break;
  ...
  case MST_EXPLICIT: {
    ...
    size_t ndx = pack_header(o_bitp, vers, MST_EXPLICIT, nbits, log2nregs,
                             expthresh, sparseon);
  ...
  case MST_COMPRESSED: {
    ...
    // compressed.
    //
    if (sparseon && ((g_max_sparse != -1 && nfilled <= (size_t)g_max_sparse) ||
                     (g_max_sparse == -1 && sparsebitsz < cmprssbitsz))) {
      size_t ndx = pack_header(o_bitp, vers, MST_SPARSE, nbits, log2nregs,
                               expthresh, sparseon);

      // Marshal the registers.
      sparse_pack(mscp->msc_regs, nbits, nregs, log2nregs, nfilled,
                  &o_bitp[ndx], i_size - ndx);
    } else {
      size_t ndx = pack_header(o_bitp, vers, MST_COMPRESSED, nbits, log2nregs,
                               expthresh, sparseon);

      // Marshal the registers.
      compressed_pack(mscp->msc_regs, nbits, nregs, &o_bitp[ndx], i_size - ndx,
                      vers);
    }
    break;
  }
  ...
  case MST_UNDEFINED:
    pack_header(o_bitp, vers, MST_UNDEFINED, nbits, log2nregs, expthresh,
                sparseon);
    break;
...
static size_t multiset_copy_size(multiset_t const *i_msp) {
  size_t retval = 0;
...
static size_t multiset_packed_size(multiset_t const *i_msp) {
  uint8_t vers = g_output_version;
...
static void check_modifiers(int32 log2m, int32 regwidth, int64 expthresh,
  ...
  if (expthresh < -1 || expthresh > expthresh_max)
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
             errmsg("expthresh modifier must be between -1 and " INT64_FORMAT,
                    expthresh_max)));
...
//
static int64 *ArrayGetInteger64Typmods(ArrayType *arr, int *n) {
  int64 *result;
  Datum *elem_values;
  int i;
...
Datum hll_typmod_in(PG_FUNCTION_ARGS) {
  ...
  if (nmods > 4 || nmods < 0) {
    ...
    typmod = (log2m << (TYPMOD_BITS - LOG2M_BITS)) |
...
Datum hll_typmod_out(PG_FUNCTION_ARGS) {
  ...
  snprintf(buffer, sizeof(buffer), "(%d,%d," INT64_FORMAT ",%d)", log2m,
...
static void multiset_union(multiset_t *o_msap, multiset_t const *i_msbp) {
  ...
  case MST_COMPRESSED: {
    ms_compressed_t *mscap = (ms_compressed_t *)&o_msap->ms_data.as_comp;
...
double gamma_register_count_squared(int nregs) {
  ...
  switch (nregs) {
  case 16:
    return 0.673 * nregs * nregs;
  case 32:
    return 0.697 * nregs * nregs;
  case 64:
    return 0.709 * nregs * nregs;
  default:
    return (0.7213 / (1.0 + 1.079 / nregs)) * nregs * nregs;
  }
}

static double multiset_card(multiset_t const *i_msp) {
  size_t nbits = i_msp->ms_nbits;
...
Datum hll_empty3(PG_FUNCTION_ARGS) {
  PG_RETURN_DATUM(DirectFunctionCall4(hll_empty4, PG_GETARG_DATUM(0),
...
Datum hll_empty2(PG_FUNCTION_ARGS) {
  PG_RETURN_DATUM(DirectFunctionCall4(
      hll_empty4, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1),
      Int64GetDatum(g_default_expthresh), Int32GetDatum(g_default_sparseon)));
}
...
Datum hll_empty1(PG_FUNCTION_ARGS) {
  PG_RETURN_DATUM(DirectFunctionCall4(
      hll_empty4, PG_GETARG_DATUM(0), Int32GetDatum(g_default_regwidth),
      Int64GetDatum(g_default_expthresh), Int32GetDatum(g_default_sparseon)));
}
...
Datum hll_empty0(PG_FUNCTION_ARGS) {
  PG_RETURN_DATUM(DirectFunctionCall4(
      hll_empty4, Int32GetDatum(g_default_log2m),
      Int32GetDatum(g_default_regwidth), Int64GetDatum(g_default_expthresh),
      Int32GetDatum(g_default_sparseon)));
}
...
Datum hll_expthresh(PG_FUNCTION_ARGS) {
  ...
  // Build the result tuple.
  {
    ...
    tuple =
        BuildTupleFromCStrings(TupleDescGetAttInMetadata(tupleDesc), values);
...
Datum hll_hash_any(PG_FUNCTION_ARGS) {
  ...
  case -2:
    hashResultDatum =
        DirectFunctionCall2(hll_hash_varlena, keyDatum, seedDatum);
    break;
  ...
  default: {
    ...
    hashResultDatum =
        DirectFunctionCall2(hll_hash_varlena, keyBinaryDatum, seedDatum);
    break;
  }
  }
...
multiset_t *setup_multiset(MemoryContext rcontext) {
  ...
  tmpcontext = AllocSetContextCreate(
      rcontext, "multiset", ALLOCSET_DEFAULT_MINSIZE, ALLOCSET_DEFAULT_INITSIZE,
